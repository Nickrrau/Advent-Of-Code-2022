...
func (m *Monkey) Test() int {
	if m.items[0]%m.test.div == 0 {
		return m.test.trueTarget
	}
	return m.test.falseTarget
}
...
func processInput(filename string) {
	...
	for scanner.Scan() {
		...
		monkeys = append(monkeys, Monkey{
			...
			op:        parseOp(op),
			test:      parseTest(test, testTrue, testFalse),
			inspected: 0,
		})
	}
...
func parseTest(test, trueTest, falseTest string) Test {
