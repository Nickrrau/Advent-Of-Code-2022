...
func (g *Grid) InsertCell(x, y int, height byte) {
	...
	if height == 'a' {
		g.startingPos = append(g.startingPos, struct{ x, y int }{x, y})
	}
...
func processInput(filename string) {
	...
	graph := Grid{values: [][]Cell{{}}, startingPos: []struct{ x, y int }{}}
	...
	shortest_length := 0
	for i := 0; i < len(graph.startingPos); i++ {
		fnd, len := FindShortestPath(
			&graph.values[graph.end.y][graph.end.x],
			&graph.values[graph.startingPos[i].y][graph.startingPos[i].x],
		)
		if fnd {
	...
	fmt.Println(shortest_length)
}

// Not the prettiest implementation of BFS, took some insporation from various examples
// including wikipedia pseudocode samples of Dijkstra's and BFS
func FindShortestPath(target, start *Cell) (bool, int) {
	visited := []*Cell{}
	queue := []*Cell{start}
